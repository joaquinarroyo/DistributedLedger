...
-import(funcSec, [maxProp/1, encolar/2, modificar/5, ordenar/2, buscarProp/2, deriverable/1, agregarSeq/2]).
...
start() ->
    ...
    register(server, spawn(?MODULE, loopServer, [[]])),
    register(deliver, spawn(?MODULE, aDeliver, [])),
    ok.

%% We end the three process
quit() ->
    node ! fin,
    server ! fin,
    deliver ! fin,
...
loopServer(Seq) ->
    receive
        %% Receive a 'send' message
        #send{msg = Msg, sender = Sender} ->
            {Tipo, _} = Msg, %% we check the type of the message
            %% If the message is a requestGet we send the sequence
            case Tipo == requestGet of
                true ->
                    {_, C} = Msg,
                    {cliente, Sender} ! {getRes, Seq, C},
                    loopServer(Seq);
                false ->
                    %% If not, we check that it is a requestAppend
                    case Tipo == requestAppend of
                        %% If it is, we add the element to the sequence
                        true ->
                            {_, {X, C}} = Msg,
                            Seq1 = agregarSeq(X, Seq),
                            if  
                                %% If the element is already in the sequence, we send the client 'nack'
                                Seq1 == Seq ->
                                    {cliente, Sender} ! {appendRes, nack, C},
                                    %% We left the sequence as it was
                                    loopServer(Seq);
                                
                                %% If we add the item, we send the client 'ack'
                                true ->
                                    {cliente, Sender} ! {appendRes, ack, C},
                                    %% We update the sequence
                                    loopServer(Seq1)
                            end;

                        %% If it is not a requestAppend, we receive a wrong message
                        false ->
                            io:format("Mensaje erroneo ~n")
                     end
            end;
...
loopBr(S, Cont, Cola, Prop, TO) ->
    receive
        ...
        %% We receive a proposal from a node
        {prop, Id, S1, Node} ->
        ...
        {consenso, Id, Node1, S1, Node2} ->
            ...
            case deriverable(Hd) of
                ...
                %% If we do not keep waiting, at some point we will order the queue in such a way
                %% that we have a 'deriverable' message in the head of the queue.
                false ->
                    case dict:size(Prop) > 0 of
                        true ->
